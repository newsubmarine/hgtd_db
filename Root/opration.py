...
class Write(SessionHandler):
    def __init__(self, type, input):
        ...
        self.temp = f"share/template/{type}_template.csv"
        self.input= input
    ...
    def update(self):
        ...
        changes = 0 
        print(f"\n {df}")
        logger.warning(f"The above row will be updated to table {self.tablename}")
        if yes_or_no():
            for data in df.itertuples(index=False):
                ...
                for c in df.columns:  # loop each columns
                    ...
                    changes = changes + 1
            print(f"You have made {changes} changes")
            Session.commit()
        else:
                logger.warning(f"Change isn't commited ")
        logger.debug(engine.execute(f"SELECT * FROM {self.tablename}").fetchall())
